...
export default function UserinfoPage() {
  ...
  const [searchOption, setSearchOption] = useState("id"); // Default search option is 'id'
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (searchTerm) {
        url += `&searchTerm=${encodeURIComponent(searchTerm)}&searchOption=${searchOption}`;
      }
      ...
      setPageInfo({
        currentPage: 1, // Set currentPage to 1 for search results
  ...
  }, [pageInfo.currentPage, searchTerm, searchOption]);
