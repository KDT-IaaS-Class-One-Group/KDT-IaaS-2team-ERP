...
app.prepare().then(() => {
  ...
  async function checkAndRenewSubscriptions() {
    ...
    const [dueSubscriptionsResult] = await pool.query(dueSubscriptionsQuery, [
      currentDate,
    ]);

    for (const subscription of dueSubscriptionsResult) {
      const { Order_Index, subs_index, user_Index, Subs_Start, Subs_End } =
        subscription;
      ...
      if (weekResult.length > 0) {
        ...
        const newEndDate = new Date(
          Subs_End.getTime() + week * 7 * 24 * 60 * 60 * 1000
        );
        ...
        await pool.query(updateSubscriptionQuery, [
          newStartDate,
          newEndDate,
          Order_Index,
        ]);
        ...
        // 갱신된 구독에 대한 로그 또는 알림
        console.log(
          `사용자 ${user_Index}의 구독이 갱신되었습니다. 시작일: ${newStartDate}, 종료일: ${newEndDate}`
        );
      }
    }
  }
